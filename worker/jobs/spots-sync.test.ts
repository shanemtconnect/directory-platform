...
import { citySpotKey, currentFeaturedSubscription, featuredForScope } from "@/lib/db/queries/spots";
...
function client(view: PayPalSubscriptionView | null | "throw", revised: number[] = []): PayPalClient {
  return {
...
describe("syncFeaturedSubscriptions", () => {
  it("does nothing at all when PayPal is not configured", async () => {
    await withTestDb(async (tx) => {
      ...
      expect(out).toEqual({ checked: 0, reconciled: 0, expired: 0, skipped: true, revalidate: [] });
