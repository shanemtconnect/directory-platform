...
export const savedSearches = pgTable("saved_searches", {
  ...
  /**
   * The watermark: the newest moment a row WENT LIVE that a digest has
   * covered — `coalesce(published_at, created_at)`, not `created_at`, because
   * a submitted listing and every job are created pending and published on
   * approval, maybe days later. Set to the moment of saving (and reset when an
   * unsubscribed search is saved again), so the first digest is what went live
   * AFTER that and not the whole back catalogue.
   */
  lastSeenPublishedAt: timestamp("last_seen_published_at", { withTimezone: true }).notNull().defaultNow(),
