...
import { JOBS_PER_PAGE, countOpenJobs, listOpenJobs, type JobFilters } from "@/lib/db/queries/job-board";
 ...
 * stored, then passed straight back to that same query with `publishedAfter`
...
/**
 * Slack on the period. The dispatch runs once an hour and the digest is
 * stamped when it is dispatched — but a stamp even a second after this
 * hour's tick would otherwise make the search due only at the NEXT tick, and
 * "daily" would become every 25 h, wandering round the clock. Anything under
 * the cron period works; half of it leaves room either side.
 */
export const DUE_TOLERANCE_MS = 30 * 60_000;
...
export async function createSavedSearch(
  ...
  // One save at a time per person, for the rest of this transaction: the cap
  // is count-then-insert, and two tabs saving at nine would otherwise make eleven.
  await tx.execute(sql`select pg_advisory_xact_lock(hashtext(${`saved_searches:${profileId}`}))`);
  ...
  const reactivate = async (): Promise<CreateSavedSearchResult | null> => {
    ...
    const [existing] = await tx
      ...
      .set({
        isActive: true,
        // Switched off from an email and saved again: alerts resume from now,
        // not with everything that went live while they were off.
        lastSeenPublishedAt: sql`case when ${savedSearches.isActive} then ${savedSearches.lastSeenPublishedAt} else ${at.toISOString()}::timestamptz end`,
        updatedAt: at,
      })
      .where(key)
  ...
  const [created] = await tx
    ...
    .values({
      ...
      // The first digest is what goes live after this moment, not the back catalogue.
 ...
 * last sent 24 h ago or more, or a weekly one 7 d ago or more — each less
 * DUE_TOLERANCE_MS, so the hourly dispatch does not drift. Only for an
...
export async function dueSavedSearches(tx: TestDb, at: Date): Promise<SavedSearch[]> {
  const dailyBefore = new Date(at.getTime() - DUE_AFTER_MS.daily + DUE_TOLERANCE_MS);
  const weeklyBefore = new Date(at.getTime() - DUE_AFTER_MS.weekly + DUE_TOLERANCE_MS);
  const rows = await tx
...
export interface Match {
  ...
  place: string | null;
  /** When it went live: `coalesce(published_at, created_at)` — what "new" and the watermark are judged on. */
  liveAt: Date;
}

/** The matches a digest shows (at most MAX_MATCHES_SCANNED), and how many there are in all. */
export type Matches = Match[] & { readonly total: number };

/**
 * What is NEW for a search: rows that went live strictly after `since`,
 * through the very query the page runs — the public viewer's, so a pending,
 * removed or expired row can never appear. "Went live", not "created": a
 * submitted listing and every job are created pending and published on
 * approval. Newest first, at most MAX_MATCHES_SCANNED; `total` is the full
 * count, for the digest's "and N more".
 */
export async function newMatchesFor(
  ...
  if (saved.kind === "listings") {
    for (let page = 1; out.length < MAX_MATCHES_SCANNED; page++) {
      ...
      for (const r of result.rows) {
        out.push({ id: r.id, title: r.name, path: `/${r.citySlug}/${r.slug}`, place: r.cityName, liveAt: r.liveAt });
      }
      if (result.rows.length < SEARCH_PER_PAGE || page >= result.totalPages) break;
    }
  } else {
    const filters: JobFilters = { ...(saved.params as JobFilters), publishedAfter: since };
    total = await countOpenJobs(tx, PUBLIC_VIEWER, filters);
    for (let page = 1; out.length < Math.min(total, MAX_MATCHES_SCANNED); page++) {
      const rows = await listOpenJobs(tx, PUBLIC_VIEWER, { ...filters, page });
      for (const j of rows) {
        out.push({ id: j.id, title: j.title, path: j.path, place: j.cityName, liveAt: j.liveAt });
      }
  ...
  const matches = out
    .sort((a, b) => b.liveAt.getTime() - a.liveAt.getTime() || a.id.localeCompare(b.id))
    .slice(0, MAX_MATCHES_SCANNED);
  return Object.assign(matches, { total: Math.max(total, matches.length) });
}
...
/**
 * After a digest: when it was dispatched (the cadence runs from there, not
 * from whenever the queue drained it), and the newest go-live it covered.
 */
export async function markSavedSearchSent(
  tx: TestDb,
  viewer: Viewer,
  id: string,
  input: { sentAt: Date; lastSeenPublishedAt: Date },
): Promise<void> {
  assertWorker(viewer);
  await tx
    .update(savedSearches)
    .set({ lastSentAt: input.sentAt, lastSeenPublishedAt: input.lastSeenPublishedAt, updatedAt: now() })
    .where(eq(savedSearches.id, id));
}

/**
 * The one-click unsubscribe. The signed token (lib/email/unsubscribe.ts) is
 * the proof, so the public viewer may do this — but only while the address
 * the token was minted for is still the owner's: after an email change the
 * old address's links stop working. The row is turned off, not deleted, and
 * saving the search again turns it back on. True only when this call changed
 * something.
 */
export async function deactivateSavedSearch(
  ...
  const rows = await tx
    ...
    .where(and(
      ...
      eq(savedSearches.isActive, true),
      sql`${savedSearches.userId} in ${owner}`,
    ))
    .returning({ id: savedSearches.id });
