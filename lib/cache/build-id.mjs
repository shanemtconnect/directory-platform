...
/**
 * Key namespace shared by every build of this app, and NOT shared with anything
 * else on the same Redis database.
 *
 * The sweep (lib/cache/sweep.mjs) SCANs `<namespace>:*` and DELs every key
 * outside the running build's prefix. Two clones pointed at the same Redis
 * database with the same namespace therefore delete each other's cache on every
 * deploy — silently, because a swept key is indistinguishable from a cold one.
 * Give each site its own Redis database index (`redis://host:6379/3`) or its own
 * `CACHE_NAMESPACE`; either is enough, both is fine.
 */
export const DEFAULT_CACHE_NAMESPACE = "nextjs";

/**
 * Same character class as a build id, and for the same reasons: the namespace
 * is the head of every key and of the glob the sweep and scripts/purge-cache.sh
 * SCAN with, so a `*`, `?` or `[` would let one site's purge match another's
 * keys and a `:` would forge an extra segment. 64 characters to match
 * VALID_PREFIX below.
 */
 ...
 * @returns {string} the configured namespace, or the default.
 * @throws if CACHE_NAMESPACE is set to something unusable. Deliberate: falling
 *   back to the default there would put this site back in the shared namespace,
 *   which is the exact collision the variable was set to avoid — and the
 *   resulting cross-site cache wipe leaves no trace. A boot that fails loudly on
 *   a value somebody typed is the better half of that trade.
 */
export function selectNamespace(env = process.env) {
  const raw = env.CACHE_NAMESPACE;
  if (typeof raw !== "string" || raw.trim() === "") return DEFAULT_CACHE_NAMESPACE;
  const trimmed = raw.trim();
  if (!VALID_NAMESPACE.test(trimmed)) {
    throw new Error(
      `Unusable CACHE_NAMESPACE: ${JSON.stringify(raw)}. ` +
 ...
 * @param {string} [namespace] defaults to `DEFAULT_CACHE_NAMESPACE`.
 * @returns {string} e.g. `nextjs:8dhlIRUNLtpabNXf4Ajbu:`
 */
export function cacheKeyPrefix(buildId, namespace = DEFAULT_CACHE_NAMESPACE) {
  ...
  if (typeof namespace !== "string" || !VALID_NAMESPACE.test(namespace)) {
    throw new Error(`Refusing to key a cache on an invalid namespace: ${JSON.stringify(namespace)}`);
  }
  return `${namespace}:${id}:`;
}
 ...
 * current one. Derived from the prefix rather than from the default namespace,
 * so it follows `CACHE_NAMESPACE` on its own — and a test can sweep a namespace
 * of its own without touching `nextjs:`.
 *
