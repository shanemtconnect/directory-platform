  ...
  global.cacheHandlerConfigPromise = (async () => {
    ...
    if (PHASE_PRODUCTION_BUILD !== process.env.NEXT_PHASE) {
      try {
        ...
        redisClient.on("error", (e) => console.warn("[spike] redis error:", e.message));
        await redisClient.connect();
        console.info("[spike] REDIS CONNECTED");
      } catch (e) {
        console.warn("[spike] redis connect failed:", e.message);
    ...
    if (!redisClient?.isReady) {
      console.error("[spike] FALLING BACK TO LRU (no redis)");
    ...
    global.cacheHandlerConfig = {
      handlers: [createRedisHandler({ client: redisClient, keyPrefix: "spike:" })],
    };
    return global.cacheHandlerConfig;
  })();
