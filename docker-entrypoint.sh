...
if [ -n "${STATIC_ASSETS_DIR:-}" ]; then
  if ! mkdir -p "$STATIC_ASSETS_DIR" 2>/dev/null; then
  ...
  # -n: never overwrite. Identical hashes are identical files; differing ones are
  # the previous deploy's, and those are the whole point.
  #
  # The source is globbed rather than written `static/.` — BusyBox cp accepts the
  # `/.` form, exits 0, and copies nothing at all, which leaves an empty volume
  # and a 404 for every asset on the page.
  if [ -d /app/.next/static ] && [ ! -L /app/.next/static ]; then
    cp -Rn /app/.next/static/* "$STATIC_ASSETS_DIR/" 2>/dev/null || true
